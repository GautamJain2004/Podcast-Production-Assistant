"""
LangGraphAgentWrapper: wraps LangGraph agents and exposes A2A-compatible handlers.
Integrates with the a2a_broker for A2A interoperability with LangGraph agents.
"""
...
logger = logging.getLogger("langgraph_adapter.adapter")
logger.addHandler(logging.NullHandler())


class LangGraphAgentWrapper:
    """
    Wrap a LangGraph agent so it can be called via A2A messages.
    The wrapper will:
    ...
    This wrapper mirrors the ADKAgentWrapper structure for consistency and enables
    LangGraph agents to participate in the A2A message-based workflow.
    """
    ...
    def __init__(self, agent_instance: Any, name: Optional[str] = None, broker_pipeline: Optional[list] = None, callback: Optional[Any] = None):
        ...
        Args:
            agent_instance: The LangGraph agent instance to wrap (must have execute method)
        ...
        self.logger = logging.getLogger(f"langgraph_adapter.{self.name}")
    ...
    def handle_a2a(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        2. Calls the wrapped LangGraph agent's execute(state) method
        ...
        try:
            ...
            if self.callback and hasattr(self.callback, 'on_agent_start'):
                try:
                    self.callback.on_agent_start(self.agent, state, framework="LangGraph")
...
def register_wrappers_with_broker(wrapped_agents: Dict[str, LangGraphAgentWrapper]) -> None:
    """
    Register LangGraph agent wrappers with the A2A broker.
    Each wrapper subscribes to its named topic and will handle incoming messages.
    
    This enables LangGraph agents to participate in the A2A message-based workflow
    alongside Google ADK agents.
    
    Args:
        wrapped_agents: Dictionary mapping agent names to LangGraphAgentWrapper instances
    """
    ...
    for name, wrapper in wrapped_agents.items():
        try:
            ...
            logger.info(f"Registered LangGraph wrapper '{name}' with broker on topic '{name}'")
        except Exception as e:
            logger.error(f"Failed to register LangGraph wrapper '{name}' with broker: {e}")
            raise
